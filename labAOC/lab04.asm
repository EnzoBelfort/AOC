...
checar_codon_parada:
  addi $s1, $s0, -1
  lb $t2, ($s1)                   # ultima base do codon

  addi $s1, $s1, -1
  lb $t3, ($s1)                  # penultima base do codon

  addi $s1, $s1, -1
  lb $t4, ($s1)                  # ante-penultima base do codon
...
AT_:
  beq $t2, 'T', exit    # ATT é válido
  beq $t2, 'C', exit    # ATC é válido
  j nao_achou_parada

AC_:
  beq $t2, 'T', exit    # ACT é válido
  j nao_achou_parada

nao_achou_parada:             # checa prox codon
  j novo_codon              
